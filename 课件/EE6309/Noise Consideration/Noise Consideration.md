...
![[Pasted image 20250225010910.png#pic_33inline|终端电阻大于特征阻抗]]![[Pasted image 20250225010923.png#pic_33inline|终端电阻小于特征阻抗]]
...
- 这在能量守恒上是很好证明的，但是很烦。但是关于相位的问题需要直接硬刚场波。
...
- 如果使用集总参数来描述这一耦合，极其困难。上图展示了一种集总参数描述耦合的情况。在这种情况中，同时存在**互感**和**互容**两种耦合方法

![[Pasted image 20250225024211.png#pic_50center|]]

- 直接把上面那一堆乱七八糟的等效成一个$Z_m$. 要注意的是这个互阻抗$Z_m$是一个虚拟的电阻，它并不在传输线中产生端口（也就意味着没有反射）
---
*这里是分析过程*
- 首先将电路图画成传输线的形式：
  ![[Pasted image 20250225024211.png#pic_50center|]]
...
- 将$V_x$左侧的传输线等效为电压为$V_1$的电压源与$Z_0$阻抗串联，$V_x$右侧等效为对地电阻$Z_0$，$V_2$左侧等效为对地电阻$Z_0$. 我们就得到了等效电路图：
