...
如果将峰值 $E_0$ 用RMS值 $E_{\mathrm{rms}}$ 代入，可得

$$P_d=\frac{E_{\text{rms}}^2}{\eta_0}$$
...
$$PL(dB)=10log_{10}\left(\frac{\lambda^2}{\left(4\pi r\right)^2}\right)=20log_{10}\left(\frac{300}{4\pi r f_{\text{MHz}}}\right)=20log_{10}\left(\frac{300}{4\pi}\right)-20log_{10}{f_{\text{MHz}}}-20log_{10}r$$
...
![[计算电场分量.png#pic_center|计算电场的投影]]

所以可以得到：

$$E_{\text{rx}}=E_{\text{inc}}cos\theta$$

需要注意的是极化损失计算的是功率上的损失，接收端接收功率与电磁波在这个位置的功率关系为：

$$P_{\text{rx}}=\frac{E_{\text{rx}}^2}{\eta_0}=P_{\text{inc}}cos^2\theta$$

其中 $\eta_0$ 是电磁波的特征阻抗， $\theta$ 代表的角度永远是接收端天线相对于电磁波的角度，由于已经处于远场所以和发射端关系不大。

以对数形式表示的话就是：

$$P_{\text{rx}}\left(\mathrm{dB}\right)=P_{\text{inc}}\left(\mathrm{dB}\right)+20log_{10}\left(cos\theta\right)$$
