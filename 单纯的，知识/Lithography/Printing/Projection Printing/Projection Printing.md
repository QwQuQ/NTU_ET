...
- 当$n=1$的时候，$NA$可以被定义为：$$NA=\mathrm{sin}\theta\approx\mathrm{tan}\theta=\frac{d/2}{f}=\frac{d}{2f}$$注意：
	...
	- NA for projector objective is also the geometrical ratio between aperture and focal length.
...
- Large NA gives smaller depth of focus.
  更大的NA会导致更浅的DOF
- This is also true for the camera. A cheap camera takes photos that are always in focus no matter where the subject is. This is because it has small lenses.
  便宜的镜头总是能对焦，因为它们的镜头很小
